...
const defaultMocks = {
  ...
  ...createMock('BIDS_QUERY', { query: BIDS_QUERY, variables: { jobId: job.id }, data: { job: { id: job.id, bids } } }),
  ...createMock('WATCH_LIST_QUERY', { query: WATCH_LIST_QUERY, data: { watchList: [{ id: job.id }] } }),
  ...createMock('TAGS_QUERY', { query: TAGS_QUERY, data: { tags } }),
  ...createMock('JOBS_SUBSCRIPTION', { query: JOBS_SUBSCRIPTION }),
  ...createMock('JOBS_RECENT_QUERY', { query: JOBS_RECENT_QUERY, variables: { lastId: null }, data: { jobs } }),
};
